...
const BillUploadDialog = ({ open, onOpenChange, onBillAdded }) => {
  ...
  const extractBillData = async () => {
    ...
    setProcessing(true);
    setOcrProgress(0);
    toast.info('Starting bill extraction...', { duration: 2000 });
    ...
    try {
      ...
      toast.info('Processing image... Please wait', { duration: 3000 });
      ...
      if (!text || text.trim().length < 10) {
        toast.error('Could not extract text. Please ensure image is clear and try again.');
      ...
      toast.error('OCR failed: ' + (error.message || 'Please enter bill details manually'));
  ...
  const parseBillText = (text) => {
    ...
    for (const pattern of datePatterns) {
      ...
      if (dateMatch) {
        try {
          ...
          console.log('Found due date:', parsed.due_date);
          break;
        } catch (e) {
    ...
    for (const pattern of accountPatterns) {
      ...
      if (accountMatch) {
        ...
        console.log('Found account number:', parsed.account_number);
        break;
      }
    }
    ...
    for (let i = 0; i < Math.min(5, lines.length); i++) {
      ...
      if (line.length > 3 && line.length < 60 && line.match(/[a-zA-Z]/) && !line.match(/^\d+$/)) {
        ...
        if (lineWords.some(word => providerKeywords.includes(word)) || 
          ...
          console.log('Found provider:', parsed.provider);
          break;
        }
      }
    }
    ...
    for (const [category, keywords] of Object.entries(categoryDetection)) {
      if (keywords.some(keyword => textLower.includes(keyword))) {
        ...
        console.log('Found category:', parsed.category);
        break;
      }
    }
    ...
    if (bpayMatch) {
      ...
      console.log('Found BPAY code:', parsed.bpay_code);
    }
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="bill-upload-dialog">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload a bill image and we'll automatically extract the details using AI
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <ul className="text-xs text-blue-800 space-y-1 list-disc list-inside">
                    ...
                    <li>Extraction takes 15-30 seconds - please wait</li>
                  </ul>
                </div>
              </div>
            )}
            ...
            {processing && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                ...
                <p className="text-sm text-yellow-600 mb-2">
                  Our AI is reading your bill. Please wait (15-30 seconds)...
                </p>
                {ocrProgress > 0 && (
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-yellow-700 mt-1 text-center">{ocrProgress}%</p>
                  </div>
                )}
              </div>
            )}
            ...
            {extracted && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <p className="text-sm text-green-600 mb-2">Review the details below and make any necessary corrections.</p>
